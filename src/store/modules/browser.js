import Vue from "vue";
import { ProPlayerBrowser } from "../../middleware/txba_helpers";
...
export default {
  ...
  mutations: {
    ...
    UPDATE_FILTER_SELECTIONS(ctx, data) {
      let resetSearchStatus = false
      console.log("update filter selections", data);
      if (!data) {
        //empty list
        resetSearchStatus = true;
        ctx.search.current.clear();
      } else if (data.group.sectionStackable === "no") {
        let current = Array.from(ctx.search.current).filter(
          element => element.group.sectionType === data.group.sectionType
        );
        if (current.length) {
          resetSearchStatus = true;
          ctx.search.current.clear();
  ...
  actions: {
    ...
    resetSearchStatus(ctx) {
      //resets status object
      const objectMap = (obj, fn) =>
        Object.fromEntries(
          Object.entries(obj).map(([k, v], i) => [k, fn(v, k, i)])
        );
      console.log("reset status")
      ctx.search.status = objectMap(ctx.search.status, v => false);
    },
    toggleSearchCriteria({ dispatch, commit, state }, itm) {
      const resetFilters = commit("TOGGLE_CURRENT_SEARCH", itm)
      console.log('itm', itm)
      if (resetFilters) {
        dispatch('resetSearchStatus')
      }else{
          state.browserTool.processDependents(itm.group.sectionId)
            .then( 
              state.search.filterList = state.browserTool.filterSectionList
              )    
      }
